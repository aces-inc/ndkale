...
Like other distributed task queue systems, publishers send task messages to queues and workers fetch messages from queues. For now, Kale supports only Amazon SQS for the queue.
...
Then the publisher publishes a task to Amazon SQS, which normally returns in about 10 milliseconds:
...
The publish() function is a static method of a task class. Other than the first parameter, which can usually be `None`, it has the same signature as the `run_task()` method. A worker process, which may run on a different machine, will pick up the message and execute the task's `run_task()` method.

While ndkale is usable out of the box, the first parameter in `publish(app_data, *args, **kwargs)` is designed for more complex situations where certain state may need to be passed outside the context of the actual task parameters. One example of this might be to pass the environment. The `app_data` must be pickleable so that it can be encoded and inserted into the SQS message.
...
We experimented and benchmarked quite a few queue selection algorithms. We use an
improved version of the lottery algorithm, **ReducedLottery**, which fulfills the above requirements.

ReducedLottery works like this:

```text
Initialize the lottery pool with all queues
while the lottery pool is not empty:
    Run a lottery based on queue priority to get a queue that wins the jackpot
...
Return whatever queue wins the jackpot
```
...
# (use this for dev to talk to ElasticMQ, which is an SQS emulator)
